...
**Upstream CI has been removed on purpose.** The vendored snapshot shipped `.github/workflows/`
(ruff, codeql, fossa, docker publish, release packaging, requirements sync) and `.github/FUNDING.yml`.
These are upstream's project-management artifacts, not ours — and `update-requirements.yml` actually
auto-committed to `hasy-integration` before it was removed. Do not restore them. If HASY ever wants CI,
write a new workflow scoped to HASY's own needs. (Side effect: `requirements.txt` was regenerated by that
bot and is inert — `uv sync` uses `pyproject.toml` + `uv.lock`.)
